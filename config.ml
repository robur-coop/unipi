...
let name =
  runtime_arg ~pos:__POS__ ~name:"name"
    {|(let doc = Cmdliner.Arg.info ~doc:"Name of the unikernel" ~docs:Mirage_runtime.s_log [ "name" ] in
...
let ssh_key =
  Runtime_arg.create ~pos:__POS__ ~name:"ssh_key"
    {|let open Cmdliner in
      let doc = Arg.info ~doc:"Private ssh key (rsa:<seed> or ed25519:<b64-key>)." ~docs:Manpage.s_arguments ["ssh-key"] in
      Arg.(value & opt (some string) None doc)|}

let ssh_password =
  Runtime_arg.create ~pos:__POS__
    {|let open Cmdliner in
     let doc = Arg.info ~doc:"The private SSH password." ~docs:Manpage.s_arguments [ "ssh-password" ] in
      Arg.(value & opt (some string) None doc)|}

let ssh_authenticator =
  Runtime_arg.create ~pos:__POS__
    {|let open Cmdliner in
     let doc = Arg.info ~doc:"SSH authenticator." ~docs:Manpage.s_arguments ["authenticator"] in
      Arg.(value & opt (some string) None doc)|}
...
let tls_authenticator =
  Runtime_arg.create ~pos:__POS__
    {|let open Cmdliner in
     ...
     in
     let doc = Arg.info ~doc ~docs:Manpage.s_arguments ["tls-authenticator"] in
     Arg.(value & opt (some string) None doc)|}
